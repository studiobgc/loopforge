...
export const DAWWorkspace: React.FC = () => {
  ...
  }, [handlePlay, isPlaying, isRecording, recordPadHit, selectedStemId, slices]);
        ...
        <div className={`flex flex-col border-r border-zinc-800 bg-zinc-900/50 transition-all ${
          ...
          {!leftPanelCollapsed && session && session.stems.length > 0 && (
            ...
            >
              ...
              <span className="text-[10px] font-normal opacity-80">Tap pads to capture your beat</span>
            </button>
          )}
              ...
              <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
                ...
                {isPlaying && isRecording && (
                  <div className="bg-red-500/20 border border-red-500/50 px-4 py-2 rounded-lg flex items-center gap-2">
                    ...
                    <span className="text-red-400/70 text-xs">Tap pads to add hits!</span>
                  </div>
                )}
                {isPlaying && !isRecording && (
                ...
                {!isPlaying && !isRecording && (
                  <div className="bg-zinc-800/90 px-4 py-2 rounded-full text-xs text-zinc-400 flex items-center gap-3 border border-zinc-700">
                    ...
                    <span><strong className="text-white">2.</strong> Tap pads to build your loop</span>
                    <span className="text-zinc-600">→</span>
        ...
        <div className={`flex flex-col border-l border-zinc-800 bg-zinc-900/50 transition-all ${
          ...
          {!rightPanelCollapsed && (
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {/* 🎛️ OUTPUT STATUS - Shows what processing is applied */}
              <div className="p-3 bg-gradient-to-r from-green-950/40 to-emerald-950/30 rounded-lg border border-green-800/30">
                <div className="flex items-center gap-2 mb-1">
                  <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                  <span className="text-xs font-semibold text-green-400">Clean Output</span>
                </div>
                <p className="text-[10px] text-zinc-500">
                  Stems play back unprocessed. No compression or effects applied.
                </p>
              </div>

              {/* 🎹 PAD BANKS - Switch between pad pages */}
              <div className="p-3 bg-zinc-800/30 rounded-lg border border-zinc-700/50">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-semibold text-zinc-300 uppercase">Pad Bank</span>
                </div>
                <p className="text-[10px] text-zinc-500 mb-2">Switch between 4 banks of 16 pads each</p>
              ...
              {/* 🎵 FEEL/GROOVE - Timing presets */}
              <div className="p-3 bg-zinc-800/30 rounded-lg border border-zinc-700/50">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <Music className="w-3.5 h-3.5 text-purple-400" />
                    <span className="text-[10px] font-semibold text-zinc-300 uppercase">Feel</span>
                  </div>
                  <span className="text-[10px] text-purple-400 font-medium">{FEEL_PRESETS[feelPreset].label}</span>
                </div>
                <p className="text-[10px] text-zinc-500 mb-2">Apply groove/swing to your pattern</p>
                <div className="grid grid-cols-2 gap-1">
                  {(Object.keys(FEEL_PRESETS) as FeelPreset[]).map((preset) => (
                    <button
                      key={preset}
                      onClick={() => {
                        setFeelPreset(preset);
                        setSwingAmount(FEEL_PRESETS[preset].swing);
                      }}
